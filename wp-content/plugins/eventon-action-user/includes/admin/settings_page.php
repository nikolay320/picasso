...
class evoau_settings{
	...
	function content_email_body_instructions(){
		return "Supported email tags that can be used in email message body: <code>{event-edit-link}</code>,<code>{event-name}</code>,<code>{event-link}</code>,<code>{event-start-date}</code>, <code>{event-start-time}</code>, <code>{event-end-date}</code>, <code>{event-end-time}</code><br/>IF available only tags: <code>{submitter-name}</code>,<code>{submitter-email}</code> --> Name and Email fields must be enabled in event submission form for these variables to work.<br/>HTML codes also can be used inside email message body.";
	}
						...
						for($t=1; $t<3; $t++){
							...
							if(!empty($ett) && !is_wp_error($ett)){
								...
								$default_tax_fields[$t][3] =array('id'=>'evoau_def_ett_v'.$ab,'type'=>'dropdown',
											'name'=>'Select default '.$_tax_names_array[$t].' tag for submitted events','width'=>'full',
										'options'=>$au_ett,);
				...
				// ARRAY
					$cutomization_pg_array = array(
						array(
							...
							'icon'=>'inbox',
							'fields'=>array(
								array('id'=>'evoau_access','type'=>'yesno','name'=>'Allow only logged-in users to submit events','legend'=>'This will allow you to only give event submission form access to loggedin users.'),
								...
								array('id'=>'evoau_ux','type'=>'yesno','name'=>'Set default user-interaction for event','afterstatement'=>'evoau_ux'),
									...
									'options'=>array(
										'1'=>'Slide Down',
										'3'=>'Lightbox',
										'4'=>'Single Event page (if single events addon installed)',
										)
									),
						...
						array(
							...
							'fields'=>array(
								...
								array('id'=>'evoau_allow_new','type'=>'yesno','name'=>__('Allow users to create new and update organizer and location fields','eventon') ),
						...
						array(
							...
							'fields'=>array(								
								...
								array('id'=>'evoau_notif','type'=>'begin_afterstatement'),
									...
									array('id'=>'evoau_ntf_admin_to','type'=>'text','name'=>'Email address to send notification. (eg. you@domain.com)', 'legend'=>'You can add multiple email addresses seperated by commas to receive notifications of event submissions.','default'=>$this->adminEmail),
						...
						array(
							...
							'fields'=>array(
								...
								array('id'=>'evo_auem_deleting','type'=>'yesno','name'=>'Allow frontend deleting events','legend'=>'This can be overridden per each event by action user settings box in event edit page'),
						)),
					);					
