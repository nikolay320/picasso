...
function pmxe_wp_ajax_wpae_filtering_count(){
	...
	if ($post['export_type'] == 'advanced') 
	{
		...
		{			
			...
			$exportQuery = eval('return new WP_Comment_Query(array(' . PMXE_Plugin::$session->get('wp_query') . ', \'count\' => true ));');
	...
	else
	{
		...
		elseif( 'comments' == $post['cpt'] )
		{
			...
			if ( version_compare($wp_version, '4.2.0', '>=') ) 
			{
				$exportQuery = new WP_Comment_Query( array( 'orderby' => 'comment_ID', 'order' => 'ASC', 'count' => true));
				$found_records = $exportQuery->get_comments();		
			}
			else
			{
				$found_records = get_comments( array( 'orderby' => 'comment_ID', 'order' => 'ASC', 'count' => true));				
			}
			remove_action('comments_clauses', 'wp_all_export_comments_clauses');			
			ob_get_clean();
		}
