 ...
 * @version 1.0.1
 */
add_filter( 'mycred_setup_hooks', 'mycred_register_gd_star_rating_hook', 60 );
function mycred_register_gd_star_rating_hook( $installed ) {

	if ( ! defined( 'STARRATING_DEBUG' ) ) return $installed;
 ...
 * @version 1.0.1
 */
add_action( 'mycred_load_hooks', 'mycred_load_gd_star_rating_hook', 60 );
function mycred_load_gd_star_rating_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'myCRED_Hook_GD_Star_Rating' ) || ! defined( 'STARRATING_DEBUG' ) ) return;
