...
class ParadoxTitanFrameworkOptionColor extends ParadoxTitanFrameworkOption {
	...
	function __construct( $settings, $owner ) {
		...
		paradox_add_action_once( 'admin_enqueue_scripts', array( $this, 'enqueueColorPickerScript' ) );
		paradox_add_action_once( 'admin_footer', array( $this, 'startColorPicker' ) );
	}
	...
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ParadoxTitanFrameworkOptionColorControl( $wp_customize, $this->getID(), array(
...
add_action( 'customize_register', 'registerParadoxTitanFrameworkOptionColorControl', 1 );


function registerParadoxTitanFrameworkOptionColorControl() {
	class ParadoxTitanFrameworkOptionColorControl extends WP_Customize_Color_Control {

		public $alpha;

		function __construct( $manager, $id, $args = array() ) {
			parent::__construct( $manager, $id, $args );
			paradox_add_action_once( 'customize_controls_print_footer_scripts', array( $this, 'printOpacityOverrideTemplate' ) );
		}
