	...
	<tr>
		<td style='<?php echo $__sty_td;?>'>			
			<div style="padding:20px; font-family:'open sans'">
				...
				<p style='<?php echo $__styles_01.$__sty_lh;?> padding-bottom:10px;'><?php echo $event_name;?></p>
				
				<?php 
					// repeat intervals for this event
					$repeat_intervals = (!empty($e_pmv['repeat_intervals']))? unserialize($e_pmv['repeat_intervals'][0]): 0;
					$this_repeat_intervals = array(0=>1);

					// for events with repeat instances 
						if( count($repeat_intervals)>0 && is_array($repeat_intervals) && !empty($e_pmv['_manage_repeat_cap_rs']) && $e_pmv['_manage_repeat_cap_rs'][0]=='yes'){	
					...
					foreach($this_repeat_intervals as $RI):
						...
						<?php
							$count_remain = $eventon_rs->frontend->functions->remaining_rsvp($e_pmv,$repeat_interval,$args['e_id']);
							$count_yes = $eventon_rs->frontend->functions->get_rsvp_count($e_pmv,'y', $repeat_interval);
							$count_no = $eventon_rs->frontend->functions->get_rsvp_count($e_pmv,'n', $repeat_interval);
							$count_maybe = $eventon_rs->frontend->functions->get_rsvp_count($e_pmv,'m', $repeat_interval);
							$count_capacity = $eventon_rs->frontend->functions->get_total_adjusted_capacity($args['e_id'],$repeat_interval,$e_pmv);
						?>
									...
									<td>
										<table style='padding-right:15px;'>
											<tr>
												...
												<td><p style="<?php echo $__sty_08;?>"><?php echo $eventon_rs->frontend->get_rsvp_status('y', $lang);?></p></td>
											</tr>
										</table>
									</td>
									<td>
										<table>
											<tr>
												...
												<td><p style="<?php echo $__sty_08;?>"><?php echo $eventon_rs->frontend->get_rsvp_status('n', $lang);?></p></td>
											</tr>
										</table>
									</td>
								</tr>
									...
									<td>
										<table style='padding-right:15px;'>
											<tr>
												...
												<td><p style="<?php echo $__sty_08a;?>"><?php echo $eventon_rs->lang('evoRSLX_email_01', 'Capacity', $lang)?></p></td>
											</tr>
										</table>
									</td>
									<td>
										<table>
											<tr>
												...
												<td><p style="<?php echo $__sty_08a;?>"><?php echo $eventon_rs->lang('evoRSLX_email_02', 'Remaining', $lang)?></p></td>
											</tr>
										</table>
									</td>
								</tr>
						...
						$guests = $eventon_rs->frontend->functions->GET_rsvp_list($args['e_id'], $repeat_interval);

						// if there are guests that are attending
						if(!empty($guests['y'])){
							echo "<div style='padding-bottom:20px;'>";
							echo $__item_p_beg. $eventon_rs->lang('evoRSL_002a', 'Guests List', $lang). ' ('.$eventon_rs->lang('evoRSL_002a1', 'Attending', $lang).")</p>";
