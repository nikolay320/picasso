...
class evo_re_functions{
	...
	// CHEKS
		// CHECK if user reviewed already
			function has_user_reviewed($post){
				...
				return ($reviewed->have_posts())? true: false;
			}
			function has_loggedin_user_reviewed(){
				...
				$review = false;
				if($reviewed->have_posts() && $reviewed->found_posts==1){
	...
	// returns
		...
		function get_rating_all_count($event_id, $event_pmv=''){
			...
			if(!$_event_reviews && count($_event_reviews)>0){ 
				return false;
			}else{
				$count = 0;		
				foreach($_event_reviews as $ri){
		...
		function get_all_reviews($event_id, $ri=0, $reviews_text=false){
	...
	// get star rating html
		function get_star_rating_html($value){
			...
			if($fraction != 0){
				$output .= "<span class='fa fa-star-half-full' data-value='{$count}'></span>";
				$count++;
			}

			for($y=1; $y<= $remain; $y++){
				$output .= "<span class='fa fa-star-o' data-value='{$count}'></span>";
				$count++;
			}
			return $output;
		}
