...
;(function ( $, window, document, undefined ) {
	...
	buildGrowl = function(base) {
		...
		$template.addClass('sabai-alert-' + base.settings.type);
	...
	addContent = function($template, settings) {
		...
		if (settings.content.message) {
			$template.find('[data-growl="message"]').html(settings.content.message).css({
				'font-size': '14px',
                'line-height': 1.5
			});
		}
		...
		update: function(command, update) {
			switch (command) {
				...
				case 'url':
					...
					break;
				case 'type':
					this.$template.removeClass('sabai-alert-' + this.settings.type);
					this.$template.addClass('sabai-alert-' + update);
					break;
				default:
