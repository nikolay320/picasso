	...
	<?php while ( bp_members() ) : bp_the_member(); ?>
		...
		<li class="kleo-masonry-item">
			<div class="member-inner-list animated animate-when-almost-visible bottom-to-top">
				<div class="item-avatar rounded <?php echo kleo_child_bp_member_get_color_class(bp_get_member_user_id()); ?>">
					<a href="<?php bp_member_permalink(); ?>"><?php bp_member_avatar(); ?></a>
					<?php do_action('bp_member_online_status', bp_get_member_user_id()); ?>
					<?php
					if ( class_exists( 'myCRED_Core' ) ) :
					?>
					<span class="kleo-mycred-rank" title="<?php echo mycred_get_users_rank(bp_get_member_user_id()); ?>"><?php echo do_shortcode( '[mycred_my_rank user_id="'.bp_get_member_user_id().'" show_title="0" show_logo="1"]' ); ?></span>
					<?php
					endif;
					?>
				</div>
