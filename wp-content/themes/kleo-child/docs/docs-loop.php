...
<?php else : ?>
	...
	<thead>
		<tr valign="bottom">
			...
			<th scope="column" style="display:none;" class="project-cell">
				Projet
			</th>
		...
		<?php while ( bp_docs_has_docs() ) : bp_docs_the_doc() ?>
			<tr<?php bp_docs_doc_row_classes(); ?>>
				...
				<td class="title-cell">
					...
					<div class="bp-docs-attachment-drawer kleo-child-bp-doc-attachment" id="bp-docs-attachment-drawer-<?php echo get_the_ID() ?>">
				...
				<?php if ( ! bp_docs_is_started_by() ) : ?>
					<td class="author-cell">
						...
						<div class="activity-avatar rounded <?php echo kleo_child_bp_member_get_color_class(get_the_author_meta( 'ID' )); ?>">
							<a href="<?php echo bp_core_get_user_domain( get_the_author_meta( 'ID' ) ); ?>">
								<?php bp_activity_avatar( 'type=thumb&user_id=' . get_the_author_meta( 'ID' ) ); ?>
							</a>
							<span class="kleo-mycred-rank" title="<?php //echo mycred_get_users_rank(get_the_author_meta( 'ID' )); ?>"><?php //echo do_shortcode( '[mycred_my_rank user_id="'.get_the_author_meta( 'ID' ).'" show_title="0" show_logo="1"]' ); ?></span>
						</div>
					</td>
				<?php endif; ?>
				...
				<td style="display:none;" class="project-cell"><?php 
						$project_id = get_post_meta ( get_the_ID(), 'bp_docs_project', true) ;
						if ( $project_id ) :
						$project = get_post( $project_id );
					?><a href="<?php echo get_site_url().'/projets/?project_id='.$project_id; ?>"><?php echo $project->post_title;?></a>
					<?php endif; ?></td>
			</tr>
		<?php endwhile ?>
	<?php endif ?>
