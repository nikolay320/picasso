...
class kleoImport {
	...
	function add_initial_demo_sets() {
		...
		$pages_data['home-get-connected'] = array(
			...
			'attach' => 'yes',
			'plugins' => array( 'bp-profile-search' ),
			//'revslider' => 'material-design',
		...
		$pages_data['home-sensei'] = array(
			...
			'attach' => 'yes',
			'plugins' => array( 'revslider', 'sensei', 'mailchimp-for-wp' ),
		...
		$pages_data['home-elearning'] = array(
			...
			'attach' => 'yes',
			'plugins' => array( 'revslider', 'mailchimp-for-wp' ),
			'revslider' => 'elearning_homepage',
		...
		$pages_data['home-portfolio-full'] = array(
			...
			'extra' => 'content/portfolio',
			'extra_name' => 'Import Portfolio posts',
			//'widgets' => 'yes',
			//'attach' => 'yes',
			'plugins' => array( 'revslider' ),
			'revslider' => 'HomeFullwidth',
		...
		$pages_data['home-stylish-woo'] = array(
			...
			//'widgets' => 'yes',
			'attach' => 'yes',
			'plugins' => array( 'woocommerce', 'mailchimp-for-wp' ),
			//'revslider' => 'HomeFullwidth',
			//'details' => 'Woocommerce plugin required.',
	...
	public function import_assets() {
		if (isset($_GET['page']) && ( $_GET['page'] == 'kleo_import' || $_GET['page'] == 'kleo-panel' ) ) {
			wp_enqueue_script( 'kleo.import', KLEO_LIB_URI . '/importer/import.js', false, KLEO_THEME_VERSION, true );
		}
					...
					<div class="demo-wrapper">
						...
						<div class="demo-options">
							<div class="to-left">
								...
								<?php
								$extra_plugin_data = '';
								if ( isset( $v['plugins'] ) && ! empty( $v['plugins'] ) ) {
									$extra_plugin_data .= 'Plugins will be installed: ';
									$extra_plugin_data .= ucwords( str_replace( '-',' ', implode( ', ', $v['plugins'] ) ) );
									$extra_plugin_data .= '<br>';
								}
								$extra_data = isset( $v['details'] ) ? $v['details'] : '';
								if ( $extra_data != '' || $extra_plugin_data != '' ) : ?>
									<span class="demo-detail"><?php echo $extra_plugin_data . $extra_data; ?></span>
								<?php endif; ?>
							</div>
	...
	public function generate_boxes_css() {
		?>
					...
					if (isset($_POST['import_demo'])) {
						...
						if (array_key_exists( $current_set, $demo_sets )) {
							...
							if (isset($_POST['import_extra']) && in_array( $current_set, $_POST['import_extra'] ) ) {
								...
								$file_path = $set_data['extra'] . '.xml.gz';
								$this->import_content( $file_path, $force_attach );
							}
